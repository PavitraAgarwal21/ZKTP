...
const EventRoute = () => {
  ...
  useEffect(() => {
    const checkEventIndex = async () => {
      try {
        setLoading(true);
        const { event_price, event_name, token_address, no_of_tickets } = {event_price : 1, event_name :"Event 1", token_address :"0x123", no_of_tickets : 100}; 
        ...
        if (event_price == 0) {
          ...
          setName(event_name);
          const t_name = "ETH" // get_token_name(token_address);
          setTName(t_name);
