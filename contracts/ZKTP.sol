...
contract ZKTP {
    ...
    function buyTicket(
        ...
        (uint256 fees, uint256 total) = calculateFees(
            ticketEvents[eventIndex].price
        );
        require(msg.value == total, "Invalid ticket price ");

        // ticket is sold out
        require(
            ticketEvents[eventIndex].noOfTicketAvl > 0,
        ...
        Address.sendValue(
            ticketEvents[eventIndex].creator,
            ticketEvents[eventIndex].price
        );
    ...
    function invalidateTicket(
        ...
        require(
            ticketEvents[TicketCommitments[_commitment].ticketEventIndex]
                .creator == msg.sender,
            "only event creator can invalidate the ticket"
        );
        require(
